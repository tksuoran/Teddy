...
/* printf(
	   "dataoutsize %d x %d, rowstride %d makes %ld bytes\n", 
	   widthout,
	   heightout, 
	   rowstride, 
	   dataoutsize
   ...
   switch (typeout) {
   case GL_UNSIGNED_BYTE:
		dataout = malloc( dataoutsize * sizeof(GLubyte) );
		if (!dataout) {
			free(tempin);
			free(tempout);
			return GLU_OUT_OF_MEMORY;
	 ...
	 for (j = 0; j < widthout * components; j++) {
      ...
      break;
   case GL_BYTE:
		dataout = malloc( dataoutsize * sizeof(GLbyte) );
		if (!dataout) {
			free(tempin);
			free(tempout);
			return GLU_OUT_OF_MEMORY;
	 ...
	 for (j = 0; j < widthout * components; j++) {
      ...
      break;
   case GL_UNSIGNED_SHORT:
		dataout = malloc( dataoutsize * sizeof(GLushort) );
		if (!dataout) {
			free(tempin);
			free(tempout);
			return GLU_OUT_OF_MEMORY;
	 ...
	 for (j = 0; j < widthout * components; j++) {
      ...
      break;
   case GL_SHORT:
		dataout = malloc( dataoutsize * sizeof(GLshort) );
		if (!dataout) {
			free(tempin);
			free(tempout);
			return GLU_OUT_OF_MEMORY;
	 ...
	 for (j = 0; j < widthout * components; j++) {
      ...
      break;
   case GL_UNSIGNED_INT:
		dataout = malloc( dataoutsize * sizeof(GLuint) );
		if (!dataout) {
			free(tempin);
			free(tempout);
			return GLU_OUT_OF_MEMORY;
	 ...
	 for (j = 0; j < widthout * components; j++) {
      ...
      break;
   case GL_INT:
		dataout = malloc( dataoutsize * sizeof(GLint) );
		if (!dataout) {
			free(tempin);
			free(tempout);
			return GLU_OUT_OF_MEMORY;
	 ...
	 for (j = 0; j < widthout * components; j++) {
      ...
      break;
   case GL_FLOAT:
		dataout = malloc( dataoutsize * sizeof(GLfloat) );
		if (!dataout) {
			free(tempin);
			free(tempout);
			return GLU_OUT_OF_MEMORY;
...
//  printf( "image %d x %d, bpp = %d makes %ld bytes\n", w, h, bpp, (w + 4) * h * bpp );
/*      image = malloc((w + 4) * h * bpp);
   ...
   while (!done) {
	 ...
	 break;
      ...
      newh = (h < 2) ? 1 : h / 2;
/*  printf(
		   "newimage %d x %d, bpp = %d makes %d rowstride and %ld bytes\n",
		   neww, 
		   newh, 
		   bpp, 
		   bpp*neww,
